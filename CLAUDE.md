...
**Ponder:** 0.17.9 · **ABIs/addresses:** `@frankencoin/zchf`  
...
- Factory-discovered contracts: positions from `PositionOpened` on MintingHub, `AmplifiedPosition` clones from `AmplifiedPositionCreated` on `UniswapAmplifier`.
- **FCS/governance contracts** (`FCS`, `MinterGovernance`, `MainnetVotes` on mainnet; `MinterGovernance`, `BridgedVotes` on all 7 L2s) share one `startFCSGovernance` block per chain — same deploy tx via `GovernanceFactory`. ABIs and addresses come from `@frankencoin/zchf`, not vendored locally.
...
| `PositionAggregatesV2History`                          | Flat time-series of aggregate snapshots, one row per block per chain                         |
...
| `FCSWrapped`/`FCSDeposit`/etc.                         | FCS (ERC-4626 vault wrapping Equity 1:1) mint/burn and deposit/withdraw events, mainnet-only |
| `MinterGovernanceMinterAnnounced`                      | Minter proposals via the governance-factory-deployed `MinterGovernance`, all 8 chains        |
| `FCSDelegation` / `BridgedVotesReceived`               | FCS vote delegation and CCIP-synced cross-chain vote totals                                  |
...
- **Untrusted contract reads** (collateral tokens, third-party oracles) go through `readWithFallback()` from `src/utils/rpc.ts`, never a bare `.catch()`. It returns the fallback only for _permanent_ failures (revert, non-contract, undecodable return) and rethrows _transient_ ones (connectivity, rate limit, timeout) so Ponder retries or restarts from checkpoint instead of persisting a fabricated value. Also wrap Frankencoin views that call into untrusted code internally: `PositionV2.availableForClones()` and `availableForMinting()` read `collateral.balanceOf(this)`, so a collateral that reverts on `balanceOf` makes them revert too. Storage-only views on Frankencoin's own contracts stay unwrapped: a revert there is a bug and must surface.
